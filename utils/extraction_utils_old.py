                    ...
                    try:
                        ...
                        if isinstance(gold_json, list):
                            ...
                            for entry in gold_json:
                                # Get the date information and normalize it
    ...
    for note_entry in tqdm(prepared_test_data, desc=f"Processing with {extractor.name}", unit="note"):
        ...
        try:
            ...
            for rel in relationships:
                ...
                # Handle both old format ('diagnosis') and new format ('entity_label')
                raw_diagnosis = rel.get('diagnosis')
                if raw_diagnosis is None:
                    # Try the new format key
                    raw_diagnosis = rel.get('entity_label')
...
def calculate_and_report_metrics(all_predictions, gold_standard, extractor_name, output_dir, total_notes_processed, dataset_path=None):
    ...
    for note_id in sorted(gold_note_ids):
        ...
        gold_str = '\n    - '.join([f"{g.get('diagnosis', 'unknown')} @ {g.get('date', 'unknown')}" for g in note_golds])
        pred_str = '\n    - '.join([f"{p.get('diagnosis', 'unknown')} @ {p.get('date', 'unknown')}" for p in note_preds])
